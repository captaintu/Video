 ...
 */
public class PictureRecorderMultiple implements IPictureRecorder {
        ...
        public PictureRecorderMultiple(String quorum, int port, String master, String auth,
    ...
    public void recordResult(PictureResult result) {
        if(mHelper != null)
        {
            ...
            try {
                ...
                //把所有是黄图的记录添加到同一张表中
                mHelper.createTable("YellowTable", new String[]{columnFamily});
                if(result.ok == true)
                {
                    mHelper.addRow("YellowTable", result.description.url, columnFamily,columns,
                            new String[]{result.description.video_id, result.description.time_stamp,
                                    String.valueOf(result.ok), String.valueOf(result.percent)});
                }
                //将记录按小时写入到不同的表中
